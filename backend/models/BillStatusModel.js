...
export const getNewesId = (result) => {
...
export const uptadeStatus = (id,result) => {
...
export const updatePaid = (id,result) => {
   db.query("UPTADE billstatus SET bill_paid = 'true' WHERE bill_id = ?",id, (err,results)=> {
     if (err){
         console.log(err);
         result(err,null);
     }else{
...
export const cancelStatus = (id,result) => {
  ...
  db.query("UPDATE billstatus SET bill_paid = 'false' WHERE bill_id = ?",id,(err,result)=> {
        if (err){
           console.log(err);
           result(err,null);
        }
